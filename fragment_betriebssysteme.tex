...
\subsection{\question{Welche zwei Hauptaufgaben hat ein Betriebssytem?}}
...
\subsection{\question{Was ist ein Prozess?}}
\begin{answer}
Ein \crucial{Programm in Ausführung}.
\end{answer}

\subsection{\question{Wie ist ein UNIX-Dateisystem strukturiert? Wie können Dateien darin (eindeutig) aufgefunden
werden?}}
\begin{answer}
...
\subsection{\question{Was ist ein \textit{symbolic link} (symbolischer Link)?}}
\begin{answer}
\paragraph*{}
...
\subsection{\question{Was ist ein \textit{hard link}?}}
\paragraph*{}
Ein \crucial{Hardlinks} ist im Grunde genommen ein \crucial{weiterer, regulärer Name der Datei}. Wird also die Datei unter ihrem ursprünglichen Namen gelöscht, ist sie unter ihrem zweiten Namen immer noch vorhanden. Intern wird dies durch Reference-Counting erreicht. Hardlinks \crucial{können nicht auf Verzeichnisse gelegt werden} (um Loops zu vermeiden).
\end{answer}

\subsection{\question{Ist das UNIX-Dateisystem wirklich ein Baum? Begründung.}}
\begin{answer}
...
\subsection{\question{Welche Zugriffsrechte kann man auf eine UNIX-Datei haben? Welche Dateiattribute steuern dies, und wie?}}
\begin{answer}
...
\subsection{\question{Welche Vorteile bietet es, auf Terminals in UNIX wie auf Dateien zuzugreifen? Was versteht man unter Ein-/Ausgabeumlenkung?}}
\begin{answer}
...
\subsection{\question{Welche Aufgabe hat ein Kommando-Interpreter (z.B. in UNIX die Shell)?}}
\begin{answer}
...
\subsection{\question{Nenne drei Beispiele für Informationen, die der Betriebsystemkern über einen Prozess wissen muss.}}
\begin{answer}
\begin{itemize}
...
\subsection{\question{Was ist eine Pipe?}}
\begin{answer}
...
\subsection{\question{Wie macht man ein soeben editiertes Shell-File ausführbar?}}
\begin{answer}
...
\subsection{\question{In welche Bereiche (Segmente) ist der virtuelle Adressraum eines Programmes in Ausführung unter UNIX unterteilt, und welche Eigenschaften kennzeichnen sie?}}
\begin{answer}
...
\subsection{\question{Wozu wird der Stack verwendet?}}
\begin{answer}
...
\subsection{\question{Welchem Zweck dienen Bibliotheken (Libraries)?}}
\begin{answer}
...
\subsection{\question{Welche Aufgabe erfüllt ein Linker?}}
\begin{answer}
...
\subsection{\question{Wozu wird beim Assemblieren eine Symboltabelle angelegt?}}
\begin{answer}
Zum Auflösen von Verweisen.
\end{answer}

\subsection{\question{Welchen Vorteil hat es, Bibliotheken mit Position Independent Code zu versehen?}}
\begin{answer}
...
\subsection{\question{Durch welche Qualitätsmerkmale soll ein Betriebssystem gekennzeichnet sein? Nenne Beispiele für konkurrierende Anforderungen.}}
\begin{answer}
...
\subsection{\question{Worin unterscheidet sich der Kernel-Mode vom User-Mode (in UNIX)? Warum wird diese Unterscheidung getroffen?}}
\begin{answer}
...
\subsection{\question{Was passiert etwa bei einem System-Aufruf?}}
\begin{answer}
...
\subsection{\question{Was ist ein Interrupt? Nenne Beispiele für mögliche Interrupt-Quellen. Warum werden sie unterschiedlich priorisiert? Wie wird ein Interrupt in etwa behandelt?}}
\begin{answer}
...
\subsection{\question{Was ist ein Trap? Nenne Beispiele für Traps. Inwiefern unterscheiden sich Traps von Interrupts?}}
\begin{answer}
...
\subsection{\question{Was ist ein Signal? Nenne Beispiele für mögliche Signalquellen. Wie kann ein Prozess auf ein Signal reagieren?}}
\begin{answer}
...
\subsection{\question{Beschreibe kurz einige Zustände, in denen sich ein (UNIX-)Prozess befinden kann.}}
\begin{answer}
Laut Wikipedia:\\
Einfaches Modell:
...
\subsection{\question{Nenne einige Randbedingungen, auf die man beim Entwurf eines Schedulers achten sollte. Wie sollten rechen-intensive bzw. Ein-/Ausgabe-intensive Prozesse dabei behandelt werden?}}
\begin{answer}
...
\subsection{\question{Wie könnte man mit Hilfe eines Round-Robin-Schedulers Prozess-Prioritäten simulieren?}}
\begin{answer}
...
\subsection{\question{Warum bestehen die Sleep-Queue und die Run-Queue in UNIX nicht aus jeweils einer
...
\subsection{\question{Warum werden die Zustandsinformationen eines UNIX-Prozesses teilweise in der PROStruktur
...
\subsection{\question{Skizziere kurz die Prozesserzeugung in UNIX.Welche Rolle spielen die Systemaufrufe fork()
und exec() dabei?}}
\begin{answer}
...
\subsection{\question{Wie erfährt ein UNIX-Prozess, ob ein Kindprozess terminiert ist? Wozu gibt es in UNIX den Prozesszustand SZOMB ("Zombie")?}}
\begin{answer}
...
\subsection{\question{Welche Vor- und Nachteile hat der First-Fit- bzw. der Best-Fit-Algorithmus zur Speicherverwaltung?}}
\begin{answer}
...
\subsection{\question{Wozu bieten Systeme eine Speicherhierachie an? Welche Beobachtung über den Speicherzugriff realer Programme liegt dem zugrunde? Welche verschiedenen Arten von Speicher
...
\subsection{\question{Warum ist es in der Regel nicht sinnvoll, den Adressraum eines Prozesses in einem Stück im Speicher abzulegen?}}
\begin{answer}
...
\subsection{\question{Was versteht man unter Paging, was unter Segmentierung? Wo tritt interne Fragmentierung, wo externe Fragmentierung auf? Was versteht man darunter?}}
\begin{answer}
Segmentierung:
...
\subsection{\question{Aus welchen Teilen besteht eine virtuelle Adresse zumeist? Wie ermittelt sich daraus die entsprechende Hauptspeicheradresse, d.h. wie läuft die Adressverwaltung in etwa ab?}}
\begin{answer}
...
\subsection{\question{Wie können mehrere Prozesse mit Hilfe virtueller Adressierung auf dieselben Programmstücke (oder auch Datenbereiche) zugreifen?}}
\begin{answer}
Shared Memory:
...
\subsection{\question{Wie arbeiten die folgenden Algorithmen zur Verdrängung von Seiten aus dem Hauptspeicher in etwa?}}
\begin{answer}
...
\subsection{\question{In welche dieser Kategorien kann man den Clock-Hand-Algorithmus einordnen?}}
\begin{answer}
LRU (ohne Dirty-Bit)
\end{answer}

\subsection{\question{Warum ist ein optimaler Algorithmus nicht realisierbar?}}
\begin{answer}
...
\subsection{\question{Was passiert, wenn die Umlaufzeit des Zeigers beim Clock-Hand-Algorithmus zu gross bzw. zu
...
\subsection{\question{Was ist Swapping?Warum wenden auch Paging-Systeme häufig dieses Verfahren an? Unter welcher Bedingung?}}
\begin{answer}
...
\subsection{\question{Wie kann man die Vorteile von Paging und Segmentierung kombinieren?}}
\begin{answer}
...
\subsection{\question{Wozu wird bei der Speicherverwaltung häufig ein Assoziativspeicher eingesetzt?}}
\begin{answer}
...
\subsection{\question{Beschreibe kurz die Zugriffsoperationen open(), close(), lseek(), read() und write() auf ein UNIXFilesystem.}}
\begin{answer}
...
\subsection{\question{Wie sieht die Struktur des UNIX-V7-Dateisystems auf der Platte in etwa aus? Warum erfolgt die Verwaltung der Freispeicherliste über Indirekt-Blöcke?}}
\begin{answer}
Das Dateisystem V7:
...
\subsection{\question{Welche Aufgaben enthält ein Inode? Welche Angaben enthält eine Verzeichnis-Datei (Directory)?}}
\begin{answer}
...
\subsection{\question{Welche Aufgaben hat der Buffer Cache in UNIX?}}
\begin{answer}
...
\subsection{\question{Was geschieht durch einen mount()-Systemaufruf in etwa?}}
\begin{answer}
...
\subsection{\question{Welche Vorteile bietet es, Dateien mit dem UNIX-Systemaufruf mmap() in den virtuellen Adressraum eines Prozesses abzubilden?}}
\begin{answer}
...
\subsection{\question{Wie ist eine Platte intern organisiert? Wie wirkt sich dies auf den Informationszugriff aus? Wie geht das UNIX Fast File System damit um?}}
\begin{answer}
...
\subsection{\question{Welche Vorteile bietet eine vereinheitlichte Betriebssystemschnittstelle für den  Zugriff auf Geräte? Wie sieht die in UNIX in etwa aus?}}
\begin{answer}
...
\subsection{\question{Was ist ein Gerätetreiber, was ein Geräte-Controller? Welche Aufgaben haben sie?}}
\begin{answer}
Gerätetreiber:
...
\subsection{\question{Warum erfolgt der Zugriff auf Geräte häufig über Warteschlangen? Wozu besitzten diese in der Regel eine High Water Mark bzw. eine Low Water Mark?}}
\begin{answer}
...
\subsection{\question{Worin unterscheidet sich Direct Memorz Access (DMA) von Programmed I/O?}}
\begin{answer}
Beim Programmed
...
\subsection{\question{Warum werden Terminal-Treiber in UNIX parametrisiert? Nenne typische Parameter.}}
\begin{answer}
