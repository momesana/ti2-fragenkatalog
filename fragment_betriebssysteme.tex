...
\paragraph*{}
Ein \crucial{Hardlink} ist im Grunde genommen ein \crucial{weiterer, regul"arer Name der Datei.} Wird also die Datei unter ihrem ursprünglichen Namen gelöscht, ist sie unter ihrem zweiten Namen immer noch vorhanden. Intern wird dies durch Reference-Counting erreicht. Hardlinks \crucial{k"onnen nicht auf Verzeichnisse gelegt werden} (um Loops zu vermeiden).
\end{answer}
...
\begin{answer}
Zunächst die Hilfe (mit dem \crucial{Kommandozeilensfwitch --help}), und wenn erforderliche die \crucial{Man-} oder \crucial{Infopages} zur Rate ziehen:
%% Bash script
...
\begin{answer}
Hängt man einem Befehl ein \& an, so wird dieser als \crucial{Hintergrundprozess} gestartet. Er gibt damit die Kontrolle über das Terminal auf, so dass dieses für neue Prozesse verwendet werden kann. Dieser \crucial{kann} dann zwar auf \texttt{stdout} \crucial{schreiben}, wird aber sofort in den Zustand \crucial{suspended} versetzt , wenn er versucht von der Standardeingabe (\textit{stdin}) zu lesen.
\end{answer}
